...
class EfficientTransformerBlock(nn.Module):
    def __init__(
        ...
        self.hidden_dim = int(
            2 * self.hidden_dim / 3
        )  # to keep param count roughly same as normal 4 * hidden_size (with swiglu we need ffn matrices instead of one)
        self.hidden_dim = self.multiple_of * (
            (self.hidden_dim + self.multiple_of - 1) // self.multiple_of
        )  # round up to multiple_of

        self.mlp_linear_13 = nn.Linear(
            self.hidden_size, self.hidden_dim * 2, bias=False
        )
...
class Dinov2ForTimeSeriesClassification(nn.Module):
    def __init__(
        ...
        self.dinov2 = torch.hub.load(
            ...
            f"dinov2_vit{self.size[0].lower()}14{'_reg' * self.use_reg}",
        )
        # print(self.dinov2)
        ...
        if self.classifier_type == "bce":
            if use_pos_weights:
                pos_weights = torch.tensor(
                    [1.0, 1.0, 1.0, 1.0]
                )  # Start unbiased, training will schedule
        ...
        if cls_option == "patches_only":
            ...
            self.context_length = (
                self.patches_per_frame * self.max_frames + self.max_frames + 1
            )
    ...
    def forward(self, x, labels=None):
        ...
        with self._backbone_context():
            out_tuple = self.dinov2.get_intermediate_layers(
                x, reshape=False, return_class_token=self.return_class_token, norm=True
            )[0]
        ...
        if self.cls_option == "patches_only":
            ...
            out = self.projection(out_tuple).reshape(
                batch_size, seq_len * self.patches_per_frame, self.embed_dim
            )
        else:  # "both"
            ...
            out = self.projection(torch.cat([cls_tokens, out_tuple[0]], dim=1)).reshape(
                batch_size, seq_len * (1 + self.patches_per_frame), self.embed_dim
            )
        ...
        out = self.transformer(
            ...
            :, 0
        ]  # Shape: [batch, embed_dim] (squeezes size 1 dim in between, transformer output is already only "1" size due to only outputting cls token, but we have to squeeze the dim)
        ...
        if labels is not None:
            last_label = labels[:, -1, :]
            loss = self.loss_fct(
                out.view(-1, self.num_classes), last_label.view(-1, self.num_classes)
            )
...
class Dinov3ForTimeSeriesClassification(nn.Module):
    def __init__(
        ...
        if use_transformers:
            ...
            weights_path_full = os.path.join(
                repo_dir, "weights", weights_path[self.size]
            )
            self.dinov3 = torch.hub.load(
                repo_dir, size2hub[self.size], source="local", weights=weights_path_full
            )

            self.dinov3 = self.dinov3.to(
                dtype
            )  # immediately convert model (tiny bit faster than doing nothing and letting autocast handle it)
        ...
        # Derived dimensions
        patch_size = 16
        self.patches_per_frame = (self.expected_input_hw[0] // patch_size) * (
            self.expected_input_hw[1] // patch_size
        )
        ...
        # Module construction
        self.projection = (
            nn.Linear(self.dinov3_embed_dim, self.embed_dim)
            if self.dinov3_embed_dim != self.embed_dim
            else nn.Identity()
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, self.embed_dim))
        ...
        MAX_FRAMES_SUPPORTED = (
            10  # Fixed value - same for all checkpoints regardless of training frames
        )
        ...
        if use_transformers:
            print(
                f"HF DINOv3 {size}: {self.dinov3_embed_dim}D, {self.num_register_tokens} register tokens"
            )
        print(
            f"DINOv3 {size}: {self.dinov3_embed_dim}D, {self.patches_per_frame} patches/frame, cls_option={cls_option}"
        )
        print(
            f"Training context: {self.max_frames} frames = ~{1 + self.patches_per_frame * self.max_frames + (self.max_frames if not self.is_patches_only else 0)} tokens"
        )
        print(
            f"RoPE supports up to {MAX_FRAMES_SUPPORTED} frames ({rope_max_seq_len} tokens) for future extension"
        )
    ...
    def _forward_patches_only(self, x, batch_size, seq_len):
        ...
        patch_tokens = self._token_extractor(
            x
        )  # [batch*seq_len, patches_per_frame, dinov3_embed_dim]
        out = self.projection(patch_tokens).reshape(
            batch_size, seq_len * self.patches_per_frame, self.embed_dim
        )
        return out
    ...
    def _forward_both(self, x, batch_size, seq_len):
        ...
        out = self.projection(combined).reshape(
            batch_size, seq_len * (1 + self.patches_per_frame), self.embed_dim
        )
        return out
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            if self.label_smoothing > 0:
                last_label = (
                    last_label * (1 - self.label_smoothing) + self.label_smoothing * 0.5
                )
    ...
    def configure_optimizers(
        ...
        hidden_params = [
            p for m in hidden_modules for p in m.parameters() if p.requires_grad
        ]
        ...
        nonhidden_params = [
            p for m in nonhidden_modules for p in m.parameters() if p.requires_grad
        ]
        ...
        print(
            f"hidden params: {hidden_num_tensors} tensors, {hidden_num_params:,} parameters"
        )
        print(
            f"Muon-optimized (hidden weights): {muon_num_tensors} tensors, {muon_num_params:,} parameters"
        )
        print(
            f"AdamW-optimized (head + gains/biases): {adam_num_tensors} tensors, {adam_num_params:,} parameters"
        )
