...
current_data_dirs = [
    config.stuck_data_dir_name,
    config.new_data_dir_name,
]  # has to be list
...
# adamw optimizer
learning_rate = (
    5e-5  # max learning rate    (when adding muon best learning area was around 2e-5)
)
...
def check_fp16_conversion_issues(model):
    ...
    for name, param in model.named_parameters():
        ...
        zeroed_values = (
            ((orig_param.abs() < min_threshold) & (orig_param.abs() > 0) & (param == 0))
            .sum()
            .item()
        )
        ...
        if max_rel_diff > 0.01 or clipped_values > 0 or zeroed_values > 0:
            ...
            print(
                f"  Original range: [{orig_param.min().item()}, {orig_param.max().item()}]"
            )
            print(f"  Max absolute difference: {max_diff}")
            print(f"  Max relative difference: {max_rel_diff * 100:.2f}%")
            if clipped_values > 0:
                print(
                    f"  {clipped_values} values likely clipped (above {clip_threshold})"
                )
            if zeroed_values > 0:
...
def load_model(sample_only=False):
    ...
    if init_from == "scratch":
        ...
        if config.use_dinov3:
            ...
            model = Dinov2ForTimeSeriesClassification(
                dino_size,
                len(id2label),
                classifier_type=classifier_type,
                cls_option=cls_option,
                use_reg=use_dino_registers,
                dropout_rate=dropout_p,
                max_frames=sequence_len,
            )
    ...
    for name, param in model.named_parameters():
        ...
        if (
            sequence_len > 1
            and freeze_non_dino_layers
            and "classifier" in name
    ...
    backbone_prefix = "dinov3" if config.use_dinov3 else "dinov2"
    backbone_params = [
        param
        for name, param in model.named_parameters()
        if name.startswith(backbone_prefix)
    ]
    model.backbone_frozen = bool(backbone_params) and all(
        not param.requires_grad for param in backbone_params
    )
    ...
    if init_from == "resume":
        try:
            ...
            print(e)
            print(
                "Probably trying to fine-tune full model after resuming from only last layer tuned model, proceeding with reset iter num for learning rate warmup."
            )
            iter_num = 0
        # iter_num = 0
...
def estimate_loss(model):
    ...
    for split in ["train", "val"]:
        ...
        if show_per_class_during_training:
            overall_accuracy, per_class_metrics = calc_accuracy(
                stacked_logits, stacked_labels, return_per_class=True
            )
            out[split + "_accuracy"] = overall_accuracy
            out[split + "_per_class"] = per_class_metrics
        else:
            overall_accuracy = calc_accuracy(
                stacked_logits, stacked_labels, return_per_class=False
            )
            out[split + "_accuracy"] = overall_accuracy
...
def print_evaluation_results(iter_num, losses_and_accs):
    ...
    print(f"{'=' * 80}")
    print(
        f"Train Loss: {losses_and_accs['train']:.4f} | Val Loss: {losses_and_accs['val']:.4f}"
    )
    print(
        f"Train Acc:  {losses_and_accs['train_accuracy'] * 100:.2f}% | Val Acc:  {losses_and_accs['val_accuracy'] * 100:.2f}%"
    )
    ...
    if show_per_class_during_training and "train_per_class" in losses_and_accs:
        ...
        if (
            classifier_type == "bce"
            and train_per_class is not None
            and val_per_class is not None
        ):
            ...
            print(
                f"{'Class':<6} {'Train Total':<12} {'Val Total':<12} {'Train Recall':<14} {'Val Recall':<12} {'Train Spec.':<12} {'Val Spec.':<10}"
            )
            print(f"{'-' * 88}")
            ...
            for i in range(len(train_total)):
                ...
                train_tot = (
                    f"{train_total[i] * 100:.1f}%"
                    if not np.isnan(train_total[i])
                    else "N/A"
                )
                val_tot = (
                    f"{val_total[i] * 100:.1f}%"
                    if not np.isnan(val_total[i])
                    else "N/A"
                )
                train_rec = (
                    f"{train_recall[i] * 100:.1f}%"
                    if not np.isnan(train_recall[i])
                    else "N/A"
                )
                val_rec = (
                    f"{val_recall[i] * 100:.1f}%"
                    if not np.isnan(val_recall[i])
                    else "N/A"
                )
                train_spec = (
                    f"{train_specificity[i] * 100:.1f}%"
                    if not np.isnan(train_specificity[i])
                    else "N/A"
                )
                val_spec = (
                    f"{val_specificity[i] * 100:.1f}%"
                    if not np.isnan(val_specificity[i])
                    else "N/A"
                )
                print(
                    f"{class_name:<6} {train_tot:<12} {val_tot:<12} {train_rec:<14} {val_rec:<12} {train_spec:<12} {val_spec:<10}"
                )
...
def plot_metrics(metrics, window_size=50):
    ...
    ax1.plot(
        range(half_window - 1, len(metrics["losses"]) - half_window),
        smoothed_losses,
        label="Smoothed Training Loss",
        color="#1f77b4",
    )
    ax1.plot(
        metrics["val_loss_iters"],
        metrics["val_losses"],
    ...
    # Plot accuracies
    ax2.plot(
        range(len(metrics["accuracy"])),
        metrics["accuracy"],
        label="Training Accuracy (Train Augmentation)",
        color="#2ca02c",
        alpha=0.3,
    )  # green
    smoothed_accuracies = moving_average(metrics["accuracy"], window_size)
    ax2.plot(
        range(half_window - 1, len(metrics["accuracy"]) - half_window),
        smoothed_accuracies,
        label="Smoothed Training Accuracy (Train Augmentation)",
        color="#2ca02c",
    )
    ...
    ax2.plot(
        metrics["val_loss_iters"],
        metrics["train_accs"],
        label="Training Accuracy (Val Augmentation)",
        color="#9467bd",
        linestyle="dashed",
        marker="s",
    )  # purple
...
def plot_class_metrics(metrics):
    ...
    for i, (ax, class_name) in enumerate(zip(axes, class_names)):
        ...
        train_total = [
            data[0][i] if data[0] is not None else np.nan
            for data in metrics["train_class_metrics"]
        ]
        train_recall = [
            data[1][i] if data[1] is not None else np.nan
            for data in metrics["train_class_metrics"]
        ]
        train_spec = [
            data[2][i] if data[2] is not None else np.nan
            for data in metrics["train_class_metrics"]
        ]

        val_total = [
            data[0][i] if data[0] is not None else np.nan
            for data in metrics["val_class_metrics"]
        ]
        val_recall = [
            data[1][i] if data[1] is not None else np.nan
            for data in metrics["val_class_metrics"]
        ...
        ax.plot(
            ...
            train_recall,
            label="Train Recall",
            color=train_colors["recall"],
            linestyle="-",
            marker=markers["recall"],
            markersize=6,
            linewidth=2.5,
            markeredgewidth=1,
            markeredgecolor="white",
        ...
        ax.plot(
            x_vals,
            train_spec,
            label="Train Specificity",
            color=train_colors["specificity"],
            linestyle="-",
            marker=markers["specificity"],
            markersize=6,
            linewidth=2.5,
            markeredgewidth=1,
            markeredgecolor="white",
        ...
        ax.plot(
            x_vals,
            val_spec,
            label="Val Specificity",
            color=val_colors["specificity"],
            linestyle="--",
            marker=markers["specificity"],
            markersize=5,
            linewidth=2,
    ...
    # Add overall title
    fig.suptitle(
        "Per-Class Performance Metrics", fontsize=16, fontweight="bold", y=0.98
    )
...
def get_batch(dataloader_iter, split):
    ...
    if device_type == "cuda":
        # DataLoader already provides pinned host memory, so only async transfer is needed here
    ...
    while True:
        ...
        if (
            ...
            if losses_and_accs["val"] < best_val_loss or always_save_checkpoint:
                best_val_loss = (
                    losses_and_accs["val"]
                    if losses_and_accs["val"] < best_val_loss
                    else best_val_loss
                )
                if iter_num > 0:
                    ...
                    state_dict = (
                        ...
                        if fine_tune
                            ...
                            if ("dinov2" not in k and "dinov3" not in k)
                        }
                    )
                    checkpoint = {
                        "model": state_dict,
                    ...
                    if best_val_loss == losses_and_accs["val"]:
                        best_ckpt_name = (
                            save_checkpoint_name.split(".")[0]
                            + "-best."
                            + save_checkpoint_name.split(".")[1]
                        )
            ...
            plot_class_metrics(
                metrics_dict
            )  # Plot per-class metrics alongside main plots
        ...
        if gradient_accumulation_steps == 1:
            ...
            for micro_step in range(gradient_accumulation_steps):
                ...
                train_logits.append(
                    logits.detach()
                )  # keep on device; move once after loop
        ...
        t0 = t1
        accumulated_loss = (
            float(loss_accum_t.item()) if torch.is_tensor(loss_accum_t) else 0.0
        )
        metrics_dict["losses"].append(accumulated_loss)
        metrics_dict["accuracy"].append(accuracy)
        if iter_num % log_interval == 0:
            print(
                f"iter {iter_num}: loss {accumulated_loss:.4f}, time {dt * 1000:.2f}ms, accuracy {accuracy * 100:.3f}%, lr {current_lr:.6f}"
            )
        iter_num += 1
        local_iter_num += 1
...
def comprehensive_eval(checkpoint_path):
    ...
    if classifier_type == "bce" and per_class_metrics is not None:
        ...
        for i in range(len(total_acc)):
            class_name = class_names[i] if i < len(class_names) else f"C{i}"
            print(
                f"{class_name:<8} {total_acc[i] * 100:>6.1f}% {true_acc[i] * 100:>7.1f}% {false_acc[i] * 100:>7.1f}%"
            )
